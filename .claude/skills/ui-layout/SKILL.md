...
```vue
<!-- ใช้ SearchBarGeneric wrapper (แนะนำสำหรับโค้ดใหม่) -->
<SearchBarGeneric
  :title="$t('view.myFeature.searchTitle')"
  :description="$t('view.myFeature.searchDesc')"
  @search="onSearch"
  @clear="onClear"
>
  <!-- #header-actions: ปุ่มมุมขวาบนใน filled header (เช่น ปุ่ม create)
       หลายปุ่มได้ — width เท่ากันทุกปุ่ม (grid 1fr), default style = btn-green
       ใช้ variant="green" เพื่อตรง semantic default; ปุ่มนี้มี label ได้ (ไม่ใช่ action bar) -->
  <template #header-actions>
    <ButtonGeneric variant="green" icon="bi-plus" :label="$t('common.btn.create')" @click="onCreate" />
  </template>

  <template #fields>
    <div>
      <span class="title-text">{{ $t('common.field.status') }}</span>
      <MultiSelectGeneric v-model="filter.status" :options="statusOptions" optionLabel="label" optionValue="value"
        :placeholder="$t('view.myFeature.placeholder.status')" :showClear="true" @update:modelValue="filter.status = $event" />
    </div>
  </template>
...
- ✅ **ปุ่มใน action bar ของหน้า list ใช้ icon-only เสมอ** — ส่งแค่ `icon` + `:title` (tooltip) ไม่ส่ง `:label` (หน้า form create/edit ยังคง label ตามปกติ)
- ✅ **`SearchBarGeneric` ต้องส่ง `:description` เสมอ** — i18n key เช่น `$t('view.myFeature.searchDesc')` อธิบายหน้านั้นสั้นๆ ห้ามเว้นว่าง
- ✅ **filter choice ใช้ `MultiSelectGeneric` เป็น default** — `data().filter` เริ่มต้นเป็น `[]`; ส่ง API `arr.length ? arr : undefined`
...
| ปุ่มใน list page action bar | `ButtonGeneric` icon-only (ไม่ส่ง `:label`) + `:title` tooltip |
| filter choice ในหน้า list | `MultiSelectGeneric` (array `[]` default, ส่ง API เฉพาะเมื่อ length > 0) |
| search bar ทุกหน้า list | `:description="$t('view.xxx.xxxDesc')"` บังคับ — ห้ามเว้นว่าง |
