...
|---|---|---|
| `InputTextGeneric` | `InputTextGeneric.vue` | wrap `<input class="form-control">` — padding sync กับ DropdownGeneric; props: `modelValue`, `type`, `placeholder`, `disabled`, `readonly`, `required`, `trim`, `bgInput`, `step`(null), `min`(null), `max`(null), `maxlength`(null); emits: `update:modelValue`, `blur`, `focus` |
