...
| `JpgDropZoneGeneric` | `JpgDropZoneGeneric.vue` | Drag & drop + click-to-pick JPG upload zone — บีบอัดด้วย `compressOptimalImage` แล้ว preview อัตโนมัติ; ควบคุมด้วย `v-if` จากฝั่ง parent เสมอ (ไม่ใช่ `v-show`) เพื่อให้ `beforeUnmount` revoke object URL กัน memory leak; props: `modelValue`(File\|null, v-model), `previewUrl`(String\|null, v-model:previewUrl), `disabled`(false), `dropLabel`(required), `browseLabel`(''), `hintLabel`(''), `formatWarningLabel`(required — ข้อความ warning เมื่อไฟล์ไม่ใช่ jpg/jpeg); emits: `update:modelValue`, `update:previewUrl`, `clear` |
