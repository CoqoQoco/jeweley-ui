...
| `ActionMenuGeneric` | `ActionMenuGeneric.vue` | ปุ่ม trigger (`ButtonGeneric`) + PrimeVue `Menu` popup (`:popup="true"`, `appendTo="body"`) — ใช้รวมปุ่มที่ทำหน้าที่คล้ายกันหลายปุ่มเป็นเมนูเดียว (เช่น header action bar); มี label → เติม chevron `bi-chevron-down` ให้อัตโนมัติ, ไม่มี label → icon-only ต้องส่ง `title`; props: `label`(''), `icon`(''), `items`(required — array ของ `{key, label, icon, command, disabled, hint, danger}`), `variant`('outline'), `disabled`(false), `title`(''); item render ผ่าน `#item` slot เอง (icon คอลัมน์คงที่ 1.25rem, `hint` บรรทัดเล็กใต้ label, `danger:true` = สีแดง, `disabled:true` = opacity .5 + command ไม่ทำงาน — PrimeVue Menu จัดการเอง); ไม่มี emits — ใช้ `item.command` ต่อรายการ |
