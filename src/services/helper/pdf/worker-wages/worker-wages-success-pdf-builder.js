  ...
  getTableContent() {
    if (this.mode === 'goldLoss') {
      return {
        ...
        table: {
          ...
          // ความกว้างเลขที่ใบงานขึ้นกับจำนวนหลักของ wo/woNumber เคยตั้งเป็นเลขตายตัวแล้วตกบรรทัดซ้ำหลายรอบ จึงให้ pdfmake วัดเองด้วย 'auto' ส่วนที่เกินไปหักจากคอลัมน์รายละเอียดซึ่งตัดบรรทัดตรงช่องว่างได้โดยไม่เสียความหมาย
          widths: [58, 'auto', 76, 30, '*', 26, 24, 36, 38, 39, 41],
    ...
    return {
      ...
      table: {
        ...
        // ฟอนต์ ChakraPetch เลขกว้างไม่เท่ากัน — 58 เผื่อพอสำหรับ DD/MM/YYYY ที่กว้างที่สุดเท่าที่เป็นไปได้ ทั้ง bold และปกติ
        // ความกว้างเลขที่ใบงานขึ้นกับจำนวนหลักของ wo/woNumber เคยตั้งเป็นเลขตายตัวแล้วตกบรรทัดซ้ำหลายรอบ จึงให้ pdfmake วัดเองด้วย 'auto' ส่วนที่เกินไปหักจากคอลัมน์รายละเอียดซึ่งตัดบรรทัดตรงช่องว่างได้โดยไม่เสียความหมาย
        widths: [58, 'auto', 77, 32, '*', 26, 42, 37, 45, 46],
