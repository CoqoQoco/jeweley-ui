import { InvoiceExcelBuilder } from './invoice-excel-builder.js'
import dayjs from 'dayjs'

export class InvoiceExcelIntegration {
  constructor() {
    this.excelBuilder = null
  }

  /**
   * Generate invoice Excel from sale order data
   * @param {Object} invoiceData - Invoice data containing items, customer, etc.
   * @param {Object} options - Excel generation options
   */
  async generateInvoiceExcel(invoiceData, options = {}) {
    try {
      ...
      // Prepare data for Excel builder
      ...
      this.excelBuilder = new InvoiceExcelBuilder(
        items,
        customer,
        invoiceDate,
        saleOrder,
      ...
      if (options.download !== false) {
        await this.excelBuilder.downloadExcel()
      }

      return {
        success: true,
        invoiceNo: invoiceNo,
        message: 'สร้าง Invoice Excel สำเร็จ'
      }
    } catch (error) {
      console.error('Error generating invoice Excel:', error)
      throw new Error(`เกิดข้อผิดพลาดในการสร้าง Invoice Excel: ${error.message}`)
    }
  }
  ...
  /**
   * Preview invoice data before generating Excel
   * @param {Object} invoiceData
   */
...
export const invoiceExcelService = new InvoiceExcelIntegration()
