<template>
  <div>
    <modal :showModal="isShow" @closeModal="closeModal" width="500px" :isShowActionPart="true">
      <template #title>
        <span class="title-text-lg px-3 pt-3 d-block">{{ $t('view.master.gemShape.createTitle') }}</span>
      </template>
      ...
      <template #action>
        <ButtonGeneric variant="main" icon="bi-save" :label="$t('common.btn.save')" type="submit" form="form-gem-shape-create" />
  ...
  methods: {
    ...
    onSubmit() {
      confirmThenSubmit(
        ...
        this.$t('view.master.gemShape.confirm.create'),
        async () => {
          await this.submit()
        }
      )
