<template>
  <div>
    <modal :showModal="isShow" @closeModal="closeModal" width="500px" :isShowActionPart="true">
      <template #title>
        <span class="title-text-lg px-3 pt-3 d-block">{{ `${$t('view.master.diamondGrade.updateTitle')}: ${model.code}-${model.nameTh}` }}</span>
      </template>

      <template #content>
        <form @submit.prevent="onSubmit" id="form-diamond-grade-update">
          <div class="p-3">
            <div class="form-row">
      ...
      <template #action>
        <ButtonGeneric variant="main" icon="bi-save" :label="$t('common.btn.save')" type="submit" form="form-diamond-grade-update" />
  ...
  methods: {
    ...
    onSubmit() {
      confirmThenSubmit(
        ...
        this.$t('view.master.diamondGrade.confirm.update'),
        async () => {
          await this.submit()
        }
      )
