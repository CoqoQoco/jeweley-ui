    ...
    <modal :showModal="isShow" @closeModal="closeModal">
      <template v-slot:content>
        <form @submit.prevent="onSubmit">
          <div class="mt-3">
            <stepperStatus :events="events" :eventsIdActive="5"></stepperStatus>
          </div>
          ...
          <div class="form-col-container mt-2">
            <div>
              <span>
                <span class="title-text">รหัสเเม่พิมพ์จริง</span>
  ...
  methods: {
    ...
    async submit() {
      try {
        ...
        params.append('cuttingBy', this.form.workBy)
        ...
        const res = await api.jewelry.post('Mold/PlanCutting', params, options)
        if (res) {
          swAlert.success(
            ``,
            '',
