    ...
    <modal :showModal="isShowModal" @closeModal="closeModal">
      <template v-slot:content>
        <div class="title-text-lg-header mb-2">
          <span>{{ $t('planUpdate.castingJobTitle') }}</span>
        ...
        <form @submit.prevent="onSubmit">
          <div class="form-col-container">
            <div>
              <span class="title-text">{{ $t('planUpdate.receiveDate') }}</span>
              <CalendarGeneric
            ...
            <div>
              <span class="title-text">{{ $t('planUpdate.receiveBy') }}</span>
          ...
          <div class="form-col-container mt-3">
            ...
            >
              ...
              <Column field="gold" :header="$t('planUpdate.goldType')" style="width: 80px">
              ...
              <Column field="requestDate" :header="$t('planUpdate.requestDate')" style="width: 210px">
                <template #editor="{ data, field }">
                  <div>
                    <Calendar
                      class="w-100"
              ...
              <Column field="goldQTYSend" :header="$t('planUpdate.goldQTYSend')" style="width: 100px">
                <template #editor="{ data, field }">
                  <input type="number" :class="data[field] ? `` : `-`" class="form-control" v-model="data[field]" />
                </template>
              </Column>
              <Column field="goldWeightSend" :header="$t('planUpdate.goldWeightSend')" style="width: 100px">
                <template #editor="{ data, field }">
                  <input type="number" step="any" :class="data[field] ? `` : `-`" class="form-control" v-model="data[field]" />
                </template>
              </Column>
              <Column field="goldQTYCheck" :header="$t('planUpdate.goldQTYCheck')" style="width: 100px">
                <template #editor="{ data, field }">
                  <input type="number" :class="data[field] ? `` : `-`" class="form-control" v-model="data[field]" @change="calTotalWages(data)" />
                </template>
              </Column>
              <Column field="goldWeightCheck" :header="$t('planUpdate.goldWeightCheck')" style="width: 100px">
                <template #editor="{ data, field }">
                  <input type="number" step="any" :class="data[field] ? `` : `-`" class="form-control" v-model="data[field]" />
                </template>
              </Column>
              <Column field="description" :header="$t('planUpdate.description')" style="width: 120px">
                <template #editor="{ data, field }">
                  <input type="text" :class="data[field] ? `` : `-`" class="form-control" v-model="data[field]" />
                </template>
              </Column>
              <Column field="workers" :header="status === 90 ? `ช่างขัด` : $t('planUpdate.workers')" style="width: 120px">
                <template #editor="{ data, field }">
                  <AutoComplete
                    v-model="data[field]"
              ...
              <Column v-if="status === 90" field="workersSub" :header="$t('planUpdate.workersSub')" style="width: 120px">
                <template #editor="{ data, field }">
                  <AutoComplete
                    v-model="data[field]"
                    :suggestions="workerItemSearch"
              ...
              <Column field="wages" :header="$t('planUpdate.wages')" style="min-width: 100px">
                <template #editor="{ data, field }">
                  <input type="number" min="1" step="any" class="form-control" v-model="data[field]" :disabled="!data.goldQTYCheck" @change="calTotalWages(data)" />
                </template>
              </Column>
              <Column field="totalWages" :header="$t('planUpdate.totalWages')" style="min-width: 120px">
                <template #editor="{ data, field }">
                  <input type="number" min="1" step="any" class="form-control" v-model="data[field]" disabled />
                </template>
              </Column>
    ...
    onSubmit() {
      if (this.validateForm()) {
        confirmThenSubmit(
          ...
          this.$t('planUpdate.confirmCasting'),
          async () => {
            await this.submit()
          }
        )
