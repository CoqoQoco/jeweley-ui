      ...
      <template v-slot:content>
        <div class="delete-reason-container">
          <div class="title-text-lg-bg">
            ...
            <span>{{ $t('view.sale.soDeposit.deleteReasonTitle') }}</span>
          </div>

          <div class="form-group mt-3">
            <label class="form-label required">{{ $t('view.sale.soDeposit.deleteReasonLabel') }}</label>
            <TextareaGeneric
              v-model="reason"
              :rows="3"
              :placeholder="$t('view.sale.soDeposit.deleteReasonPlaceholder')"
            />
          </div>
...
export default {
  ...
  methods: {
    onConfirm() {
      ...
      if (!trimmed) {
        warning(
          this.$t('view.sale.soDeposit.validation.deleteReasonRequired'),
