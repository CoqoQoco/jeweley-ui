...
|------|---------|--------|
| ลบใบสั่งขาย | `!isViewMode` | ⚠️ **ยังไม่ implement** — มีปุ่มแต่ไม่มี handler (ดู PLAN.md) — ใช้ `SaleOrder/Inactive` (Status=0) แทน delete. Mobile: ยังไม่มีปุ่ม |
...
| Export PDF | - | `SaleOrderPdfBuilder` + inline `pdfShowCifLabel` toggle |
...
### API: POST `/SaleOrder/Inactive` ⚠️ ยังไม่ implement — ดู PLAN.md

Set SO `Status = 0, StatusName = "Inactive"` — ซ่อนจาก List (soft delete)

**Service**: `SaleOrderService.Inactive()` (ต้องสร้างใหม่)
**Pinia Store**: `usrSaleOrderApiStore.fetchInactive()` (ต้องสร้างใหม่)
**Frontend**: Web (`sale-order-view.vue`) + Mobile (`detail-view.vue`)

**Business Rules**:
- มี `TbtSaleOrderProduct.Invoice != null` → error (มี Invoice อยู่ ห้าม inactive)
- มี confirmed items (`Invoice = null`) → **auto-unconfirm ทั้งหมดก่อน** แล้ว inactive (1 transaction)
- `List()` เพิ่ม default filter `Status > 0` → inactive SO ไม่ขึ้น list

**DB Changes** (ใน 1 transaction):
- `TbtSaleOrderProduct` (Invoice=null): hard delete + restore `TbtStockProduct.QtySale`
- `TbtSaleOrder.Status = 0`, `StatusName = "Inactive"`
...
**UI Toggle Pattern** — inline checkbox ข้างปุ่ม PDF (ไม่ใช่ใน modal):

| View | data property | ส่งไปที่ |
|------|--------------|---------|
| `sale-order-view.vue` | `pdfShowCifLabel: true` | `SaleOrderPdfBuilder(options.showCifLabel)` — ทุก call site (exportPDF, previewPDF) |
| `quotation-view.vue` | `pdfShowCifLabel: true` | `previewInvoice()` โดยตรง + `ConfirmCreatePdfView` ผ่าน `:defaultShowCifLabel` prop → modal emit `showCifLabel` ให้ `onConfirmItemsPerPage` + `onSaveAndCreatePdfAndSave` |

`ConfirmCreatePdfView` modal: รับ `defaultShowCifLabel` prop → pre-fill checkbox ใน modal (user ยัง override ได้ใน modal ก่อน confirm)
...
| POST | `/SaleOrder/Inactive` | ⚠️ ยังไม่ implement — Inactive SO (Status=0) + auto-unconfirm |
...
- **Inline C.I.F checkbox** (data: `pdfShowCifLabel: true`) อยู่ข้าง "Preview Quotation" — เปิด/ปิด C.I.F label สำหรับทุก PDF ใน view นี้
- `previewInvoice()` — ข้าม `ConfirmCreatePdfView` modal, เรียก `generateInvoicePdf()` ตรงด้วย `showCifLabel: this.pdfShowCifLabel`
- `ConfirmCreatePdfView` modal รับ `:defaultShowCifLabel="pdfShowCifLabel"` — pre-fill ค่าใน modal ตาม inline toggle
