# Design System — Source of Truth

เอกสารนี้คือ **มาตรฐานกลางของการออกแบบ UI** ในโปรเจกต์ jeweley-ui — ทุกหน้า/ทุก feature ต้องยึดตามนี้

> **บังคับ**: ก่อน implement/ออกแบบ UI ต้องอ่านเอกสารนี้ก่อน และทุกครั้งที่เพิ่ม/เปลี่ยน design pattern ใหม่ → ต้องบันทึกใน [Design Decision Log](#design-decision-log) ท้ายไฟล์
>
> เอกสารนี้เป็น **source-of-truth** (หลักการ + baseline) — รายละเอียดเชิงลึกอยู่ใน skill (ไม่ copy ซ้ำ):
> `design-system`(tokens) · `ui-layout`(layout) · `native-call-policy`(generic) · `i18n-system` · `generic-components`

---

## Purpose

สถาปนา design language เดียวกันทั้งระบบ เริ่มจากหน้า **Ticket — แจ้งปัญหา/ขอฟีเจอร์** เป็น baseline แล้วใช้ refactor หน้าอื่นต่อ เป้าหมาย: สะอาด, focus, เรียบง่าย, consistent, reusable

---

## Core Principles

| # | หลักการ | รายละเอียด |
|---|---|---|
| 1 | **Page header มาตรฐาน** | ใช้ `PageHeaderGeneric` (back + title) — ปุ่มรอง (เช่น คู่มือ) วางใน `#actions` slot เท่านั้น ห้ามปุ่มลอยในการ์ด |
| 2 | **แบ่งเป็นหลาย box ตาม logic** | จัด field เป็นกลุ่มตามความหมาย แต่ละกลุ่ม = `SectionCardGeneric` **มี border + title ของตัวเอง** (ไม่รวมทุกอย่างในการ์ดใบเดียว) เพื่อสร้าง focus — **หัวข้อกล่อง/section ต้องมาจาก `SectionCardGeneric` (แนะนำ `headerStyle="legend"` + icon + accent เหมือนหน้า gold-loss-tang) เท่านั้น — ห้ามประดิษฐ์แถบ box-title/section-header เอง** |
| 3 | **Label แหล่งเดียว** | `FormFieldGeneric` เป็นที่มาของ label เพียงแหล่งเดียว — component ลูก (เช่น upload) **ห้าม render title/label ซ้ำ** |
| 4 | **Multi-column = grid** | แถวหลายคอลัมน์ใช้ mixin `form-row-grid(n)` (responsive ≤1024px ยุบเป็น 1 คอลัมน์) ห้ามเขียน media query เอง |
| 5 | **Footer action bar** | ปุ่ม action หลักอยู่ **นอก box** ชิดขวา, primary ขวาสุด, เว้นด้วย `ml-2` (Bootstrap 4) ห้าม flex gap |
| 6 | **Token เท่านั้น** | สี/ระยะ/มุม/เงา ใช้ `var(--sp-*)`,`var(--radius-*)`,`var(--shadow-*)`,`var(--base-*)`,`var(--color-*)` — ห้าม hardcode px/hex (ดู `design-system` skill); **บน filled surface** (พื้น maroon เช่น top bar/drawer/page header/modal main) ใช้ `var(--overlay-white-*)` / `var(--on-inverse-*)` แทน rgba มือ — ดูหัวข้อ [Filled / Inverse Surface](#filled--inverse-surface) |
| 7 | **Generic component เท่านั้น** | ห้าม native `<input>`/`<button>`/`<table>`/`<img>` หรือ PrimeVue ตรงๆ — ใช้ generic wrapper (ดู `native-call-policy` skill); รูป Azure ใช้ `ImagePreview` |
| 8 | **i18n เท่านั้น** | ห้าม hardcode ข้อความไทย ใช้ `$t()` เสมอ (ดู `i18n-system` skill) |
| 9 | **Reusable first** | ถ้าต้องการ UI ซ้ำ 2+ ที่ → สร้าง generic component ใหม่ (เช่น `MultiImageUpload`) อย่า inline ซ้ำ |
| 10 | **List page buttons** | ปุ่มใน search/action bar ของหน้า list: **ปุ่ม primary search ใส่ label ได้** (เช่น 'ค้นหา') เพื่อให้ action หลักเด่น — **ปุ่มรอง (clear/export/advanced/create) คง icon-only + `:title`** tooltip เท่านั้น (ไม่ส่ง `:label`) — ref: `/plan-order-tracking` |
| 11 | **Filter = MultiSelect เป็น default** | filter field ที่เป็น choice ใช้ `MultiSelectGeneric` เสมอ **ห้ามใช้ `DropdownGeneric`** เว้นแต่ field นั้นเลือกได้ค่าเดียวโดยธรรมชาติจริงๆ (ระบุเหตุผลใน Decision Log) |
| 12 | **Page title มี description เสมอ** | ทุก `SearchBarGeneric` ต้องส่ง prop `description` อธิบายหน้านั้น (i18n) — ห้ามมีแต่ title เปล่า |
| 13 | **Chart palette** | กราฟทุกตัวใช้สีจาก `CHART_PALETTE` (`services/utils/chart-colors.js`, 5 สี mirror จาก `variable.scss`) เท่านั้น — ถ้าจำนวนชุดข้อมูลเกิน 5 ให้ยุบส่วนที่เหลือเป็น "อื่นๆ" หรือเปลี่ยนรูปแบบกราฟให้ใช้สีเดียว (เช่น ranking bar) **ห้ามเพิ่มสีใหม่เข้า palette เพื่อให้พอกับจำนวนชุดข้อมูล** |
| 14 | **ห้ามแถบสีหนาด้านซ้าย** | กล่องสถานะ / callout / แถบแจ้งเตือน ห้ามใช้ `border-left` หนาหรือเทคนิคใดๆ ที่สร้างแถบสีด้านซ้ายเป็นตัวเน้น — สื่อสถานะด้วย **ไอคอน + สีตัวอักษร + กรอบบาง 1px เท่ากันทุกด้าน** (หรือพื้นอ่อนจาก token) เท่านั้น — user สั่งถอดแบบนี้มาหลายครั้ง (slide-menu, SourceStripGeneric, setting/barcode-printer) |

---

## Reference Layout (canonical — หน้า Ticket create)

```
┌────────────────────────────────────────────────────────────────────┐
│ (←)  <Page Title>                                       [ปุ่มรอง...] │  PageHeaderGeneric + #actions
└────────────────────────────────────────────────────────────────────┘
  ┌─ Box 1 · <กลุ่ม logic 1> (border + title) ───────────────────────┐
  │  Label A *        Label B *        Label C *                      │  form-row-grid(n) inline
  │  [____]           [▼ ____]         [____]                         │  (≤1024px ยุบ 1 คอลัมน์)
  │  Label D (full width)                                             │
  │  [______________________________________________]                │
  └────────────────────────────────────────────────────────────────────┘
  ┌─ Box 2 · <กลุ่ม logic 2> (border + title) ───────────────────────┐
  │  <fields / MultiImageUpload / ...>                                │
  └────────────────────────────────────────────────────────────────────┘
                                          [ยกเลิก]  [➤ บันทึก]  footer นอก box ชิดขวา
```

**ไฟล์อ้างอิงจริง**: `src/views/ticket/create-view.vue`

---

## Reference Layout — Dashboard (canonical — หน้า Ticket manage)

```
┌────────────────────────────────────────────────────────────────────┐
│ (icon) <Dashboard Title>                        [ตัวกรอง▾] [⟳]      │  DashboardHeaderGeneric (+ slot #controls, ปุ่ม refresh)
└────────────────────────────────────────────────────────────────────┘
  [●12]     [●5]      [●7]      [●92%]     [●3]                        .kpi-grid (grid, ไม่มีกล่อง/ไม่มี title ครอบ)
  ป้าย A    ป้าย B    ป้าย C    ป้าย D     ป้าย E                     → StatCardGeneric ×N (สูงเท่ากันทุกใบ, ≤1024px ยุบ 2-3 คอลัมน์)
  ┌─ กราฟ A (legend) ─────────────┐  ┌─ กราฟ B (legend) ─────────────┐
  │ SectionCardGeneric            │  │ SectionCardGeneric            │  section 2 คอลัมน์ (`.charts-row-b`)
  │ headerStyle="legend" + icon   │  │ headerStyle="legend" + icon   │  grid-template-columns: 1fr 1fr
  │ + accent                      │  │ + accent                      │  (≤1024px ยุบเป็น 1 คอลัมน์)
  └────────────────────────────────┘  └────────────────────────────────┘
┌────────────────────────────────────────────────────────────────────┐
│ SearchBarGeneric — filter fields + ปุ่มค้นหา/ล้าง                     │
└────────────────────────────────────────────────────────────────────┘
┌────────────────────────────────────────────────────────────────────┐
│ DataTableWithPaging — รายการเต็มความกว้าง                            │
└────────────────────────────────────────────────────────────────────┘
```

**ไฟล์อ้างอิงจริง**: `src/views/ticket/manage/components/ticket-dashboard.vue` (header + kpi-grid + charts 2 คอลัมน์), ประกอบรวมกับ `SearchBarGeneric`/`DataTableWithPaging` ที่ `src/views/ticket/manage/index-view.vue` — `DashboardHeaderGeneric`/`StatCardGeneric` ถูกสกัด (genericize) มาจากไฟล์นี้เป็น generic component กลาง (ดู Decision Log 2026-07-21)

หมายเหตุ: ถ้าหน้า dashboard ต้องแบ่ง KPI เป็นหลายกลุ่มพร้อมหัวข้อกำกับ (เช่น กลุ่ม PLAN แยกจากกลุ่ม SLIP) ให้ครอบแต่ละกลุ่มด้วย `SectionCardGeneric headerStyle="dashboard"` (หัวข้อ + เส้นคั่น เท่านั้น ไม่มีกรอบ/พื้นครอบ) แทนการปล่อย `.kpi-grid` ลอยไม่มีหัวข้อ — กราฟ/ตารางใน dashboard เดียวกันยังใช้ `headerStyle="legend"` ตามเดิม ดู `src/views/production/gold-loss-dashboard/components/overview-tab-view.vue` และ Decision Log 2026-09-14

---

## Spacing Rhythm (baseline)

| ตำแหน่ง | ค่า token |
|---|---|
| padding ภายใน box | `var(--sp-xl)` |
| เว้นระหว่าง box | `var(--sp-lg)` |
| เว้นระหว่าง field ใน box | `var(--sp-lg)` |
| เว้นปุ่มใน footer | `ml-2` |

---

## Icon Input

กฎ: icon ที่อยู่ภายในช่องกรอกข้อมูล (leading icon) ต้องใช้ `InputTextGeneric` พร้อม prop `:icon` เท่านั้น

- icon ฝังอยู่ใน control เดียวกับ input (inline) — ไม่ใช่กล่อง addon แยก
- ใช้ Bootstrap Icon class name เช่น `'bi-telephone-fill'`, `'bi-envelope-check-fill'`
- ห้ามเขียน `<div class="input-group">` + `.input-group-append` + `.input-group-text` manual อีก

```vue
<!-- ✅ Good — icon ผ่าน prop -->
<InputTextGeneric id="tel1" type="tel" icon="bi-telephone-fill" v-model.trim="form.tel1" />
<InputTextGeneric id="email" type="email" icon="bi-envelope-check-fill" v-model.trim="form.email" />

<!-- ❌ Bad — input-group manual (พัง: icon ลอยเหนือ input) -->
<div class="input-group input-group-inner">
  <div class="input-group-append">
    <span class="input-group-text"><i class="bi bi-telephone-fill"></i></span>
  </div>
  <InputTextGeneric type="tel" v-model="form.tel1" />
</div>
```

---

## Modal Form Standard

modal form ที่มีหลาย field ต้องแบ่งเป็นกลุ่มตาม logic ดังนี้:

1. **title bar** ใช้ `<modal headerVariant="main">` — bg สี main (`--base-font-color`) + text/✕ ขาว ให้ title เด่น
2. แต่ละกลุ่ม logic ใช้ `SectionCardGeneric` (กล่อง border + title) เหมือน page form (Core Principle #2) — เว้นระหว่างกล่อง `var(--sp-lg)`
3. ใช้ `InputTextGeneric` พร้อม prop `icon` สำหรับ field ที่มี icon (ห้าม input-group manual)
4. spacing ใช้ token `var(--sp-*)` ทั้งหมด — ห้าม hardcode px
5. footer (บันทึก/ยกเลิก) อยู่ใน `#action` slot ของ `modal-view.vue` เสมอ (นอกกล่อง)

```
╔════════════════════════════════════════════════════════════════════════════════╗
║  แก้ไขข้อมูลลูกค้า                                               (ขาว)  [ ✕ ] ║  ◀ bg = main, text/✕ ขาว
╠════════════════════════════════════════════════════════════════════════════════╣
│  ┌─ ข้อมูลหลัก ───────────────────────────────────────────────────────────┐   │
│  │ รหัสลูกค้า *  [201901](disabled)   ประเภทลูกค้า * [▼ L: ในประเทศ      ✕] │   │
│  │ ชื่อภาษาไทย * [เดบิว           ]     ชื่อภาษาอังกฤษ [Debut             ]  │   │
│  │ ที่อยู่ติดต่อ  [BKK (textarea) ]     หมายเหตุ      [(textarea)         ]  │   │
│  └────────────────────────────────────────────────────────────────────────┘   │
│  ┌─ ช่องทางติดต่อ ────────────────────────────────────────────────────────┐   │
│  │ เบอร์โทร 1   [☎  0930515544  ]      เบอร์โทร 2   [☎                 ]    │   │
│  │ E-mail      [✉  debutbkk@..  ]      บุคคลติดต่อ  [👤  Art            ]   │   │
│  └────────────────────────────────────────────────────────────────────────┘   │
│  ┌─ การขาย ──────────────────────────────────────────────────────────────┐   │
│  │ ส่วนลด (%)  [0]                                                          │   │
│  └────────────────────────────────────────────────────────────────────────┘   │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                  [ 💾 บันทึก ]   [ ยกเลิก ]     │
└──────────────────────────────────────────────────────────────────────────────┘
```

SCSS มาตรฐานสำหรับเว้นระหว่างกล่อง (scoped ใน modal):

```scss
.modal-section {
  margin-bottom: var(--sp-lg);

  &:last-child {
    margin-bottom: 0;
  }
}
```

Section cards inside modals should use the legend header style (`headerStyle="legend"` with matching `icon` and `accent`) to match the visual language of page forms — this applies to both create/edit modals and detail/read-only modals. Legend boxes reserve top space (`margin-top: var(--sp-2xl)`) for the straddling chip, and box padding must remain uniform via `card-base` (`var(--sp-xl)`) — do not override padding per box.

**✅ Good:**
```vue
<modal headerVariant="main" ...>
  <template #title>
    <span class="title-text-lg d-block">หัวข้อ Modal</span>
  </template>
  <template #content>
    <SectionCardGeneric :title="$t('view.x.section.main')" headerStyle="legend" icon="bi-card-list" accent="main" class="modal-section">
      <!-- form rows -->
    </SectionCardGeneric>
    <SectionCardGeneric :title="$t('view.x.section.contact')" headerStyle="legend" icon="bi-card-list" accent="main" class="modal-section">
      <!-- form rows -->
    </SectionCardGeneric>
  </template>
</modal>
```

**❌ Bad:**
```vue
<!-- เส้นใต้ h6 เดิม (เลิกใช้แล้ว) -->
<h6 class="form-section-title">ข้อมูลหลัก</h6>

<!-- title bar ธรรมดาบนพื้นขาว -->
<span class="title-text-lg px-3 pt-3 d-block">หัวข้อ</span>

<!-- รวมทุก field กล่องเดียว -->
<SectionCardGeneric title="ข้อมูลทั้งหมด">...</SectionCardGeneric>

<!-- ใส่ padding เองที่ root ของ #content (modal-view เป็นเจ้าของระยะแล้ว) -->
<template #content>
  <div class="p-3">
    <SectionCardGeneric ...>...</SectionCardGeneric>
  </div>
</template>
```

**Reference implementation**: `src/views/customer/list-customer/modal/create-view.vue`

---

## Modal Spacing Standard

`modal-view.vue` (shared modal component) เป็นเจ้าของระยะห่างภายในกล่อง modal **แหล่งเดียว** — แบ่งเป็น 4 โซน แต่ละโซนใช้ token คงที่ ไม่ขึ้นกับ consumer:

| โซน | Element | Token |
|---|---|---|
| Header | `.base-modal__header` | `padding: var(--sp-md) var(--sp-lg)` |
| ✕ (close button) | `.float-close` | `top: var(--sp-sm); right: var(--sp-sm)` |
| Content | `.content-container` | `padding: var(--sp-xl)` (หรือ `0` เมื่อ `contentPadding="none"` ผ่าน `.content-container--flush`) |
| Footer | `.base-modal__operation` | `padding: var(--sp-lg) var(--sp-xl)` |

**consumer ห้ามใส่ padding ที่ root ของ `#content` เอง (ห้าม `p-3`/`p-4`/scss padding) — modal-view เป็นเจ้าของระยะเดียว; เนื้อหา full-bleed ใช้ `contentPadding="none"`**

```vue
<!-- ✅ Good — ไม่มี wrapper padding, ให้ modal-view จัดการระยะเอง -->
<modal :showModal="isShow" @closeModal="$emit('closeModal')">
  <template #content>
    <SectionCardGeneric title="...">...</SectionCardGeneric>
  </template>
</modal>

<!-- ✅ Good — เนื้อหา full-bleed (รูป/PDF/iframe เต็มกรอบ) ใช้ contentPadding="none" -->
<modal :showModal="isShow" contentPadding="none" @closeModal="$emit('closeModal')">
  <template #content>
    <div class="receipt-preview-wrap">...</div>
  </template>
</modal>

<!-- ❌ Bad — ใส่ p-3 เองที่ root ของ #content (ซ้ำซ้อนกับ .content-container) -->
<template #content>
  <div class="p-3">...</div>
</template>
```

**Reference**: `src/components/modal/modal-view.vue`, `src/components/public/product-share-dialog.vue`, `src/views/customer/list-customer/modal/create-view.vue`

---

## Filled / Inverse Surface

พื้น maroon เต็ม (filled) ที่ใช้ซ้ำใน top bar, navigation drawer, page-title filled header (`SearchBarGeneric`), และ modal `headerVariant="main"` — codify เป็น token + mixin แทนการเขียน rgba/gradient มือ (เดิมค่าเพี้ยนกันเองระหว่างหน้า)

**ใช้เมื่อไหร่**: top bar (`main-bar.vue`), navigation drawer (`.custom-sidebar`, `side-bar.vue`), page-title filled header (`page-title.vue` / `SearchBarGeneric`), `PageHeaderGeneric`, `DrawerGeneric` (`headerVariant="main"`), modal `headerVariant="main"`

**Canonical gradient**: panel/drawer = **แนวตั้ง** `--surface-inverse-gradient` (`linear-gradient(180deg, ...)`), แถบ/bar แนวนอน (top bar, section header bar) = **แนวนอน** `--surface-inverse-gradient-bar` (`linear-gradient(90deg, ...)`) — ห้ามใช้มุมทแยง (135deg) กับ core surface ใหม่ (มุมทแยงเหลือเฉพาะ mobile/one-off เดิม รอ migrate รอบถัดไป)

**Dot texture**: `--surface-dot` (`radial-gradient(rgba(255,255,255,0.08) 1px, transparent 1.5px)`) — canonical alpha = **0.08** (เดิม drawer ใช้ 0.06, page-title ใช้ 0.08 — รวมเป็นค่าเดียว ต่างกัน ≤0.02 มองไม่ออก)

### Overlay-white scale (ใช้บน filled surface แทน rgba มือ)

| Token | ค่า | ใช้เมื่อ |
|---|---|---|
| `--overlay-white-subtle` | rgba(255,255,255,0.06) | rest/expanded state อ่อน |
| `--overlay-white-hover` | rgba(255,255,255,0.08) | hover |
| `--overlay-white-chip` | rgba(255,255,255,0.12) | chip/pill พื้น |
| `--overlay-white-solid` | rgba(255,255,255,0.15) | icon box, border, active-emphasis, divider |
| `--overlay-white-strong` | rgba(255,255,255,0.25) | active counter, scrollbar thumb |

### Text tiers บน filled surface

| Token | ค่า | ใช้เมื่อ |
|---|---|---|
| `--on-inverse` | #ffffff | label หลัก |
| `--on-inverse-muted` | rgba(255,255,255,0.85) | label รอง |
| `--on-inverse-dim` | rgba(255,255,255,0.65) | icon/arrow |
| `--on-inverse-label` | rgba(255,255,255,0.5) | section label |

### Inner element บน filled surface — กติกา 2 สำนวน

| สำนวน | ใช้ที่ | ตัวอย่าง |
|---|---|---|
| **Rose-gold active** | เมื่อ inner element ต้องการ active state เด่น (ไม่กลืนพื้น maroon) | drawer/nav route active pill (`var(--nav-active-gradient)`) — teal ยังใช้กับ secondary/status/zone อื่นตามเดิม |
| **White outline** | เมื่อ inner element เป็นปุ่ม action บนพื้น filled ทั่วไป | `PageHeaderGeneric` back button/actions (border/color `--on-inverse`, hover พลิกเป็นพื้นขาว) |

### Mixin

```scss
@import '@/assets/scss/mixin.scss';

// gradient แนวตั้ง + dot (ค่า default) — ใช้กับ drawer/sidebar
.custom-sidebar {
  @include filled-surface;
}

// flat + dot — ใช้กับ page-title filled header
.page-title-filled {
  @include filled-surface($gradient: false, $dot: true);
}

// flat ล้วน ไม่มี dot — ใช้กับ DrawerGeneric is-main
.drawer-header.is-main {
  @include filled-surface($gradient: false, $dot: false);
}
```

ดูรายละเอียด token/mixin เพิ่มเติมใน `design-system` skill

---

## Design Decision Log

> บันทึกทุกครั้งที่ออกแบบ/เปลี่ยน design pattern ใหม่ (บังคับโดย hook `remind-on-change.sh`)

| วันที่ | หน้า/Feature | สิ่งที่เปลี่ยน / pattern ใหม่ |
|---|---|---|
| 2026-06-22 | ticket/create-view | สถาปนา baseline design system: multi-box แยก border, `PageHeaderGeneric` + `#actions` slot, `MultiImageUpload` (generic ใหม่), footer action bar นอก box, label แหล่งเดียวผ่าน `FormFieldGeneric` |
| 2026-06-22 | PageHeaderGeneric (global) | เพิ่ม `border-top: 2px solid var(--base-font-color)` ให้ page header bar — กรอบบน-ล่างเข้มสมมาตร |
| 2026-06-22 | PageHeaderGeneric (global) | เปลี่ยน page header เป็น filled bg สี main (`--base-font-color`) + inner elements (title/back/actions) เป็นสีสว่าง (white-outline auto ใน actions slot) |
| 2026-06-22 | ticket/create-view + router | เพิ่ม **tab/segment-switch pattern** (ButtonGeneric active=main, inactive=outline) รวมหลายมุมมองในหน้าเดียว (แจ้งใหม่ + Ticket ของฉัน); เมนูกลุ่มที่มี in-page toggle ให้ใช้ลิงก์ตรง (`minorShow: false`) เพื่อ discoverability |
| 2026-06-24 | list-page standard (global) | **#10 icon-only buttons**: ปุ่ม search/clear/create/export ในหน้า list ไม่ส่ง `:label` — ใช้แค่ `icon` + `:title` tooltip; ฟอร์ม create/edit ยังคง label ตามเดิม |
| 2026-06-24 | list-page standard (global) | **#11 MultiSelect default**: filter choice ทุกตัวในหน้า list ใช้ `MultiSelectGeneric` (array, chip display) แทน `DropdownGeneric` (single); `data().filter` ต้องเริ่มต้นเป็น `[]`; ส่ง API เฉพาะเมื่อ `array.length > 0` |
| 2026-06-24 | list-page standard (global) | **#12 description บังคับ**: `SearchBarGeneric` เพิ่ม prop `description` (String, default '') และส่งต่อให้ `pageTitle` — ทุก list page ต้องใส่ `:description="$t('view.xxx.xxxDesc')"` (i18n key ทั้ง th และ en) |
| 2026-06-25 | InputTextGeneric (global) | เพิ่ม prop `icon` — icon-input ฝังในช่อง (leading), เลิก input-group manual ที่ icon ลอย; migrate ทีละหน้าในรอบถัดไป |
| 2026-06-25 | customer/create-view (modal) | Modal Form Standard: title bar headerVariant=main (filled bg), แต่ละกลุ่มใช้ SectionCardGeneric (border box + title) แทนเส้นใต้ h6, footer นอกกล่อง — reference customer create-view |
| 2026-06-30 | SectionCardGeneric (global) | เพิ่ม opt-in headerStyle='legend' (title คร่อมเส้น border บน + icon + accent main/green, ไม่มีเส้นใต้) — backward compatible, default ยังเป็น pageTitle underline; ใช้ครั้งแรกที่หน้า gold-loss-tang |
| 2026-06-30 | SectionCardGeneric (global) | headerStyle='legend': เปลี่ยน `.section-legend` เป็น **outlined chip** — เพิ่ม `border: 1px solid` (accent color), `border-radius: var(--radius-md)`, padding บน-ล่าง `var(--sp-xs)` (จากเดิม `0`) ให้ title อยู่ในกล่องมีกรอบสีตาม accent มุมมน คร่อมเส้น border บนของ card |
| 2026-06-30 | variable.scss (global) | เพิ่ม token `--color-green-bg: #e6f4f4` (teal tint อ่อน, SCSS `$color-green-bg` + CSS custom property) — ใช้กับ zone สรุป/คำนวณที่ต้องการ focus เป็นพื้นเขียวอ่อน (teal) แทน highlight-bg (ชมพู) |
| 2026-06-30 | gold-loss-tang / summary-panel | `.calc-summary` เปลี่ยน background เป็น `var(--color-green-bg)` + border เป็น `var(--base-green)` — สร้าง visual focus ที่ zone สรุป&คำนวณ; remark TextareaGeneric แยกลงแถวล่างเต็มกว้าง (จาก three-col เป็น two-col + แถวล่าง full-width) |
| 2026-07-06 | layout/slide-menu | Work vs system menu grouping ผ่าน `meta.menuSection: 'system'` (route-level, ไม่แตะ logic); กลุ่ม system (ตั้งค่าระบบ/Project Graph/ดาวน์โหลด) ได้ gold (`--base-warning`) left-accent 3px + warm tint bg + section label — แยกโซนจากกลุ่มงาน translucent-white เดิม |
| 2026-06-30 | gold-loss-tang / issued+returned lines | เปลี่ยน `FormFieldGeneric` ต่อแถว → **div-grid table** `grid-template-columns: 1fr 140px 44px` ให้ทุกแถว (base/header/input/total) อยู่แนวเดียวกัน; ค่าน้ำหนักจาก 4dp → **2dp** ทุกจุด (display เท่านั้น — logic calc ไม่เปลี่ยน); step input 0.0001 → 0.01 |
| 2026-06-30 | SearchBarGeneric/pageTitle (global) | search bar header → filled main ตัวอักษรขาว (prop `filled` ใน `page-title.vue`, non-breaking); input focus ring (`border-color: var(--base-font-color)` + `box-shadow: 0 0 0 3px rgba(146,19,19,.08)`); status chip teal (`var(--color-green-bg)` bg + `var(--base-green)` text); **#10 ปรับ**: primary search ใส่ label ได้ — ปุ่มรอง (clear/export/advanced/create) คง icon-only + `:title` — blueprint: docs/claude-design/blueprints/search-bar.md |
| 2026-06-30 | SearchBarGeneric filled header (global) | เพิ่ม slot `#header-actions` (มุมขวาบน header filled แดง) — รองรับหลายปุ่ม, width เท่ากัน (grid `auto-columns: 1fr`), default style `btn-green` (var(--base-green)); ปุ่ม create ย้ายจาก action bar มาที่นี่, caller ใช้ `variant="green"` เพื่อตรง semantic default |
| 2026-07-01 | SearchBarGeneric/pageTitle (global) | adopt ref บางส่วน: (#1) filled header เพิ่ม dot texture `radial-gradient(rgba(255,255,255,.08) 1px, transparent 1.5px)` size 14px; (#2) MultiSelect chip เปลี่ยนเป็น outline teal (`background: transparent` + border/text `var(--base-green)`); (#3 ไม่รับ) ปุ่ม list page คง icon-only ตาม #10 — ไม่รับ ref แบบ label segmented |
| 2026-07-02 | gold-loss-tang detail modal | Detail/read-only modal ใช้ Modal Standard เต็ม: headerVariant="main" (title bar filled) + section cards ใช้ headerStyle="legend" (icon+accent main/green) ให้สไตล์ box title ตรงกับหน้า create — reference gold-loss-tang list/modal/detail-view |
| 2026-07-02 | SectionCardGeneric (global) | headerStyle='legend': เพิ่ม margin-top var(--sp-2xl) ให้ chip ที่คร่อมขอบบน (top:0/translateY(-50%)) ไม่ชนขอบ box ก่อนหน้า; padding ทุก box ต้องเท่ากันจาก card-base (var(--sp-xl)) ห้าม override |
| 2026-07-06 | variable.scss (global) | เพิ่ม status color tokens (`--status-open/-progress/-resolved/-closed/-cancelled` + คู่ `-bg`) — mirror สีจาก dashboard donut เดิม ให้ table badge กับ dashboard chart ใช้แหล่งสีเดียวกัน แก้ปัญหาสถานะเดียวกันแสดงคนละสีระหว่างหน้า |
| 2026-07-06 | ticket/manage (list+detail+dashboard) | box-title เขียนเอง (`.panel-section-header`/`.panel-divider`) → แปลงเป็น `SectionCardGeneric headerStyle="legend"` ทุกกล่อง (detail panel ซ้าย 2 กล่อง, การ์ดฝั่งขวา 3 กล่อง, chart การ์ด dashboard 4 กล่อง); สร้าง shared `ticket-status-badge.vue`/`ticket-type-badge.vue` + `ticket-status.js` constants แทน array/method ซ้ำ 3 ไฟล์ |
| 2026-07-07 | main-bar / side-bar (global nav) | nav bar + drawer active state เปลี่ยนจาก teal (`--base-green`) → rose-gold pill (`--nav-active-gradient` ไล่เฉด `#edc2b3`→`#c97f6d`, text/icon `--nav-active-on` `#3d1712`, glow `--nav-active-glow`); top bar background เปลี่ยนเป็น `@include filled-surface` (vertical gradient + dot) ให้เป็นผืนเดียวกับ drawer; เพิ่ม token `--rose-gold-*` + `--nav-active-*`; teal ยังใช้กับ secondary/status/zone อื่นตามเดิม |
| 2026-07-06 | layout/slide-menu (global) | redesign navigation drawer จากโทนเทา hardcode → **maroon filled-surface + teal active accent**: `.custom-sidebar` bg gradient แนวตั้ง `var(--base-font-color)`→`var(--base-font-sub-color)` + dot texture (เหมือน page-title-filled); เมนู item ทุกระดับ = translucent-white (rest `.06`/hover `.12`/expanded `.15`); route active accent เปลี่ยนจาก maroon → `var(--base-green)` (teal) กันกลืนพื้น; counter badge แดง (`--base-red`) เมื่อมีค่า; logout = red tint; ทุก spacing/radius → token; i18n header (`sidebar.title` namespace ใหม่) + logout (`common.btn.logout`) + `showMenuName()` locale-aware; ลบ dead code `.social-banner` — blueprint: docs/claude-design/blueprints/slide-menu.md |
| 2026-07-06 | design-system tokens (global) | ยก "filled maroon surface" pattern (top bar/drawer/page-title filled header/modal headerVariant="main") เป็น token กลาง: `--surface-inverse`/`--surface-inverse-gradient`/`--surface-inverse-gradient-bar`/`--surface-dot`, `--overlay-white-*` (5 tiers), `--on-inverse-*` (4 tiers) + `@mixin filled-surface`; adopt ใน `main-bar.vue`/`side-bar.vue`/`page-title.vue`/`PageHeaderGeneric.vue`/`DrawerGeneric.vue`/stock product update modal — ไม่เปลี่ยนหน้าตา (canonical dot alpha 0.08 แทน 0.06 เดิมของ drawer — ต่างกันมองไม่ออก) |
| 2026-07-06 | layout/slide-menu (global) | **supersede** gold-accent grouping ด้วย **3 section labels + dividers + flat muted items** (ตาม ref ผู้ใช้ — คง maroon bg เดิม): item ทุกระดับ bg `transparent`(rest)→`rgba(255,255,255,.08)`(hover)→`rgba(255,255,255,.06)`(expanded), text ~85%/icon ~65% ขาว; ลบ `.is-system` (gold left-border/tint) ทั้งหมด; เพิ่ม `meta.menuSection: 'data'` ให้ customer/worker/report-production-worker-wages/master (system เดิมคงไว้); template คำนวณ `sectionOf(index)`/`showSectionHeader(index)` render `.menu-section-label` (uppercase muted) + `.menu-section-divider` เมื่อกลุ่มเปลี่ยน (แทนที่ `tpLineShow` divider เดิม กันเส้นซ้อน); active pill คง teal (`--base-green`) + `border-radius: var(--radius-md)`; i18n `sidebar.section.{main,data,system}` แทน `sidebar.sectionSystem` — blueprint: docs/claude-design/blueprints/slide-menu.md |
| 2026-07-06 | layout/slide-menu | เพิ่ม 3-level menu hierarchy (L1 main/L2 sub/L3 children) หลังจาก flatten ทำให้แยกชั้นไม่ออก: L1 label ขาวเต็ม (`--on-inverse`) weight 600 + parent เปิดอยู่ bg เข้มขึ้น (`--overlay-white-hover`); L2 เยื้อง `--sp-md` + left rail 2px (`--overlay-white-solid`) + text 0.85rem dim (`--on-inverse-dim`); L3 เยื้องลึกกว่า + left rail 1px จางกว่า (`--overlay-white-hover`) + text 0.8rem dim สุด — teal active pill ยัง override สีจางทุกระดับ, SCSS เท่านั้น ไม่แตะ behavior — blueprint: docs/claude-design/blueprints/slide-menu.md |
| 2026-07-21 | dashboard genericization Phase 0 (global) | สร้าง dashboard building blocks กลาง สกัดจาก ticket-dashboard.vue: `StatCardGeneric` (KPI tile, variant main/warning/green/grey), `DashboardHeaderGeneric` (icon+title/subtitle+slot #controls+ปุ่ม refresh), `ChartGeneric` (ApexCharts wrapper — merge default options กับ props.options ด้วย lodash merge, built-in loading/empty state) + `chart-colors.js` (CHART_TOKENS/CHART_PALETTE mirror token เป็น hex — apexcharts อ่าน CSS var ไม่ได้) — ยังไม่แตะ consumer ใดๆ (ticket-dashboard.vue คงเดิม), รอ migrate ใน phase ถัดไป |
| 2026-08-02 | layout/slide-menu | ยก L2/L3 ของ drawer เป็น `--on-inverse` ขาวเต็ม + section label เป็น `--on-inverse-muted` w600 — ลำดับชั้นสื่อด้วยขนาด/น้ำหนัก/เยื้อง/rail แทนความจาง (แก้ปัญหา sub-menu อ่านยาก + section label ไม่ผ่าน AA) |
| 2026-08-02 | layout/slide-menu | เพิ่ม token `--overlay-dark-recess` (rgba(0,0,0,.2)) + ใช้ที่ `.submenu-container` ทำโซน sub-menu เป็นพื้นเว้าเข้ม ให้ตัวอักษรขาวเด้งขึ้น (contrast 9.1→11.2:1) — additive token ไม่แก้ค่าเดิม ไม่แตะ filled-surface/top bar |
| 2026-08-31 | plan-gold (create) + plan-tracking-gold/update-view (edit modal) | Phase 3: ยุบไฟล์ซ้ำ ~90% เป็น shared section components `plan-gold/components/{gold-info,melt,cast}-section.vue` + `body-return-table.vue` + `balance-panel.vue` (prop `mode: 'create'\|'edit'` คุม disable เล่มที่/เลขที่) — `index-view.vue` 1046→275 บรรทัด, `update-view.vue` 909→323 บรรทัด; หัวข้อกล่อง `.title-text-lg-bg` ×3 → `SectionCardGeneric headerStyle="legend" accent="main"`, แถบขี้เบ้า teal ทึบ → nested `SectionCardGeneric accent="green"`; ตารางคืนตัวเรือนเปลี่ยนจาก `editMode="row"` เป็นแถวแก้ไขได้ตลอด (ตัด rowEditor/editingRows) + เพิ่ม empty state; รวม i18n `planTrackingGold`→`planGold` namespace เดียว (65 key th/en ตรงกัน); เพิ่ม `FormFieldGeneric` prop `inputId` (optional, backward compatible) ผูก `<label for>` ให้ครบ 30 ช่อง — blueprint: docs/claude-design/blueprints/plan-gold-form.md |
| 2026-08-31 | plan-gold Phase 3 follow-up fix | (1) แก้ regression: 17 จุดที่ใช้ `Number($event)` cast ช่องว่าง `''`→`0` แทน `null` (backend `decimal?` เพี้ยนความหมาย) → เพิ่ม shared helper `toNullableNumber()` ใน `decimal.js` ใช้แทนทั้ง 17 จุด (`cast-section`/`melt-section`/`body-return-table`/`gold-info-section`); (2) a11y `<label for>` ชี้ผิดที่ 8 จุด (id ตกที่ PrimeVue wrapper `.p-calendar`/`.p-dropdown` ไม่ใช่ `<input>` จริง) → เพิ่ม prop `inputId` (optional, backward compatible) ให้ `CalendarGeneric`/`DropdownGeneric`/`AutoCompleteGeneric` forward ตรงไปยัง PrimeVue `inputId` prop ที่ผูก `<input>` จริงอยู่แล้ว; เพิ่ม prop `ariaLabel` ให้ `AutoCompleteGeneric` (forward `aria-label` ไปยัง input จริงเช่นกัน) ใช้กับ input ซ้ำในตาราง (WO/จำนวน/น้ำหนัก/รายละเอียด) ที่ไม่มี id เดี่ยว |
| 2026-09-08 | Public showcase (/p/:token) | เพิ่ม customer-facing surface แรกของระบบ — exempt จาก admin chrome (`PageHeaderGeneric`/`SearchBarGeneric`/`SectionCardGeneric`) ตามที่ตกลงไว้ (ลูกค้าสแกน QR ต้องไม่เห็น sidebar/topbar); ยังคงบังคับ token+i18n+generic component เหมือนเดิม; ใช้ axios instance แยก (`axios/axios-public.js`) ไม่มี interceptor เพราะ endpoint public ต้องไม่โชว์ swAlert modal/ไม่ redirect ไป /login เมื่อ error; ช่องทางติดต่อ (Facebook/IG/Email/Website) รวมไว้ที่ `config/public-contact-config.js` จุดเดียว |
| 2026-09-08 | invoice-detail header | จัดกลุ่มปุ่ม header จาก 9 ปุ่ม `btn-green` สีเดียวกันหมด (ตกบรรทัดบนจอ 1366–1600px) เหลือ 5 control แถวเดียว: `เพิ่ม Version`, `พิมพ์ใบแจ้งหนี้` (primary filled ตัวเดียว), `เอกสารอื่น ▾`/`Excel ▾`/`⋯` (เมนู popup); header เปลี่ยนจาก `.card`/`.card-header` custom → `PageHeaderGeneric`; สร้าง generic ใหม่ `ActionMenuGeneric` (ปุ่ม `ButtonGeneric` trigger + PrimeVue `Menu` popup, item slot เอง รองรับ hint/danger/disabled); `PageHeaderGeneric` เพิ่ม modifier `.page-header-actions :deep(.btn.is-primary)` (filled ขาว ตัวอักษร `--base-font-color`) + `.page-header-bar { flex-wrap: wrap }` กันตกขอบจอแคบ; ปุ่มใบรับประกันที่ปิดอยู่ (ยังไม่ชำระครบ) ย้ายจาก tooltip บน `<span>` ครอบ → เมนูไอเท็ม disabled + hint บรรทัดเล็ก; ยกเลิกเอกสาร (destructive) ย้ายเข้าเมนู ⋯ เป็นรายการสีแดง — blueprint: `docs/claude-design/blueprints/invoice-detail-header-actions.md` |
| 2026-09-09 | stock-product list + detail | row actions 5→2 (`ButtonGeneric` `main bi-eye` ดูรายละเอียด / `outline bi-pencil` แก้ไข — ปุ่มพิมพ์ป้าย/ต้นทุน/ประวัติ/QR ย้ายไป header หน้า detail); row expand (`data-expand-view.vue`) จาก `h6` เขียนเอง + fix `width: 750px` + hex/px hardcode → 2 `SectionCardGeneric headerStyle="legend"` (วัตถุดิบซ้าย/ยอดคงเหลือขวา) ใน `.expand-container { position: sticky; left: 0 }` (แทนการ fix width เดิม); หน้าใหม่ `/stock-product-detail/:stockNumber` (archetype C Detail/View) — `PageHeaderGeneric` (4 ปุ่ม ghost: พิมพ์ป้าย/ต้นทุน/ประวัติ/QR + ปุ่มแก้ไข `is-primary`) + 5 `SectionCardGeneric legend`, โหลดเองจาก `StockProduct/Get` (deep-link/refresh ได้), ฝัง modal เดิม 5 ตัวไม่แก้ไฟล์; สกัด shared components ใหม่ `src/views/stock/product/components/material-table.vue` (variant compact/full) และ `balance-panel.vue` (ล็อตนี้ 3 tile + ตาราง sloc) ใช้ร่วมกันระหว่าง expand กับหน้า detail; ยุบ `mergeBalanceIntoItems()` ที่ซ้ำเป็น composable `src/composables/useStockBalanceMerge.js` — blueprint: `docs/claude-design/blueprints/stock-product-detail-view.md` — gotcha: `td` ของ `.p-datatable-row-expansion` มี `overflow: hidden` จาก PrimeVue theme ทำให้ sticky ไม่ทำงาน ต้อง `:deep(.p-datatable-row-expansion > td) { overflow: visible }`; i18n `materialsTitle` มี `|` ถูก vue-i18n ตีเป็น plural (เห็นแค่ "เพชร") → แก้เป็น `{'|'}` literal |
| 2026-09-08 | modal-view (global) | Modal Spacing Standard: `modal-view.vue` เป็นเจ้าของระยะห่างภายในกล่อง modal แหล่งเดียว — header `padding: var(--sp-md) var(--sp-lg)`, ✕ `top/right: var(--sp-sm)`, content `.content-container { padding: var(--sp-xl) }` (ใหม่ — เดิมไม่มี padding เลย), footer `.base-modal__operation` จาก hardcode `20px` → `var(--sp-lg) var(--sp-xl)`; เพิ่ม prop `contentPadding` (`'default'\|'none'`) → class `.content-container--flush` สำหรับเนื้อหา full-bleed (รูป/PDF/iframe); migrate consumer 46+ ไฟล์ทั่วระบบ (ลบ `p-0..5`/`px-*`/`py-*`/`pt-*`/`pb-*`/`pl-*`/`pr-*` ที่ root ของ `#content`) — reference: `product-share-dialog.vue`, `customer/list-customer/modal/create-view.vue`; full-bleed exception: `receipt/receipt-print-action.vue` (`contentPadding="none"`) |
| 2026-09-08 | announcement (feed widget) + dashboard/home | empty state แบบเต็มกล่อง + CTA เฉพาะผู้มีสิทธิ์ + quick-actions กลับมาที่หน้าแรก |
| 2026-09-08 | announcement (feed widget + manage page, ใหม่) | ฟีเจอร์ประกาศข่าว: (1) `announcement-card.vue` — compact clamp 3 บรรทัด (`-webkit-line-clamp`) / full ใช้ `white-space: pre-line`, thumbnail 88px square ใช้ raw `<img>` แทน `ImagePreview` เพราะ `ImagePreview` bind prop `style` (border/radius/object-fit) เข้า root `<span>` ของ PrimeVue Image ไม่ใช่ตัว `<img>` จริง ทำ `object-fit: cover` ผ่านมันไม่ได้ (ใส่ comment อธิบายในโค้ดตาม native-call-policy), whole-card clickable (`role="button"` + `@keydown.enter`); (2) pinned tag = bordered chip (`border: 1px solid var(--base-warning)`, ไม่ใช่พื้นสีทึบ) ต่างจาก status-badge ปกติที่เป็นพื้นทึบ; (3) feed บนหน้าแรกใช้ **load-more button** แทน pagination (`ButtonGeneric outline` โชว์เฉพาะ `items.length < total`, emit `load-more` ให้ store append); (4) `SectionCardGeneric headerStyle="legend"` ไม่รองรับ slot `#header-actions` (มีเฉพาะ `filled`) — ปุ่ม "จัดการประกาศ" ย้ายเป็น toolbar row right-align ในเนื้อหาแทน; (5) filter สถานะ/ปักหมุดในหน้า `/announcement` ใช้ `DropdownGeneric` (ไม่ใช่ `MultiSelectGeneric` ตาม Core Principle #11) เพราะเป็นค่าที่เลือกได้ครั้งละหนึ่งค่าโดยธรรมชาติ (สถานะ/ปักหมุด mutually exclusive ไม่ใช่ multi-choice) — ยกเว้นตามเงื่อนไขที่ระบุไว้ใน #11; (6) status badge หน้า list reuse token เดิมจาก ticket module แทนเพิ่ม token ใหม่: `Visible→--status-resolved`, `Hidden→--status-closed`, `Scheduled→--status-open`, `Expired→--status-cancelled` — blueprint: `docs/claude-design/blueprints/announcement-card.md` |
| 2026-09-09 | announcement (audience field) | เพิ่ม field `audience` (all/dev) ควบคุมการมองเห็นบนหน้าแรก + tag กลาง "เห็นเฉพาะ Dev" วางถัดจาก pinned tag ในการ์ด/detail modal/list — blueprint: `docs/claude-design/blueprints/announcement-card.md` |
| 2026-09-08 | invoice-detail payment zone | redesign โซนชำระเงิน+สรุปยอด: (1) แก้ overflow ทั้งหน้า — `.form-content-payment-container` เป็น `grid: 1fr 4fr` แต่ grid item ไม่ได้ตั้ง `min-width: 0` (computed `auto` หดต่ำกว่า min-content ไม่ได้) ทำตารางประวัติชำระเงินดันทั้งหน้าล้น (`scrollWidth` 1854 vs viewport 1521) → เพิ่ม `> * { min-width: 0 }` + breakpoint 1024px เป็น 1 คอลัมน์ — **กฎทั่วไป: grid item ที่มีตารางกว้างต้องตั้ง `min-width: 0` เสมอ มิฉะนั้น container จะบีบไม่ลงและดันหน้าให้ล้น**; (2) `invoice-items-table.vue` ตัด `ColumnGroup type="footer"` แถว `colspan=16` ทั้งหมด (ส่วนลดพิเศษ→ยอดที่ต้องชำระ, 9 แถว) ที่ทำให้เห็นเป็น "แถวว่าง" เวลาไม่เลื่อนสุดขวา (เนื้อหาอยู่ที่ x≈2136px บนตารางกว้าง 2287px) — เหลือเฉพาะแถวยอดรวมคอลัมน์จริง (Net Weight/Total); (3) เขียนใหม่ `payment-section.vue`: `SectionCardGeneric headerStyle="legend"` + แถบสถานะ (badge/progress bar ตาม `getPaymentStatus()`/`getOutstandingAmount()` จาก `services/utils/payment-status.js` — คืน `null` เมื่อไม่รู้ยอดรวม ห้ามเดาว่าค้างชำระ) + กล่องยอดเงิน `--color-green-bg` (320px) + ปุ่ม "บันทึกรับเงิน" ย้ายจาก card-header มาอยู่ในแถบสถานะ (emit ใหม่ `record-payment`) + ประวัติชำระเปลี่ยนจากตาราง 13 คอลัมน์ (`BaseDataTable`) เป็น card list (thumbnail สลิป + ฟิลด์รองแสดงเฉพาะที่มีค่า) — คง emit `delete-payment` เดิม; (4) component ใหม่ `money-summary-card.vue` — สรุปยอดแบบใบเสร็จ พับได้ (ปิดไว้ default, `ButtonGeneric variant="plain"` + chevron toggle), เลิกซ้ำเลข "ยอดที่ต้องชำระ" ที่เคยโผล่ 3 ที่ (footer ตารางสินค้า/กล่องสรุป/ท้ายตารางชำระเงิน) เหลือที่เดียวต่อ context — blueprint: `docs/claude-design/blueprints/invoice-detail-payment-zone.md` |
| 2026-09-10 | sale/invoice-detail (⋯ menu) + ActionMenuGeneric | เมนู danger: group header + separator + icon chip tint แดง + hover/focus ผูก token; รายการยกเลิกแยกเป็น 2 ทาง (ยกเลิกใบเดียว / ยกเลิก + ปลดยืนยันสินค้าใน SO) |
| 2026-09-11 | gold-loss-dashboard (ใหม่) + TabViewGeneric/SourceStripGeneric | รวม 6 report เดิม (by-stage/by-worker/tang-slip/setter-slip/reconcile/monthly) เป็น dashboard แท็บเดียว แยกตามต้นทางข้อมูล (PLAN/SLIP/BOTH) — สร้าง generic ใหม่ 2 ตัว: (1) `TabViewGeneric` ครอบ PrimeVue TabView/TabPanel — ซ่อน nav header เดิม (`display:none`) แล้ว render nav bar เอง พร้อม group divider (label กำกับกลุ่ม `จากแผนผลิต (PLAN)`/`จากใบ (SLIP)`/`รวม`); lazy ทำเองด้วย `visited` Set (ไม่พึ่ง PrimeVue `:lazy` เพราะ unmount ทุกครั้งที่สลับแท็บออก ทำ filter ภายในแท็บหาย + fetch ซ้ำ); (2) `SourceStripGeneric` แถบคาดบังคับทุกแท็บประกาศต้นทาง (ตาราง/หน้าที่คีย์/นับอะไร) + ปุ่ม ⓘ ขยายคำอธิบายยาว ผูก enum `source: plan\|slip-tang\|slip-setter\|both` เข้า i18n โดยตรง (ไม่มี slot ข้อความ กันลืมใส่คำอธิบาย); shared filter (ช่วงวันที่/แผนก/ช่าง) อยู่นอกแท็บ ค่าคงอยู่เมื่อสลับแท็บ — field ที่บางแท็บไม่ใช้ (เช่น แผนกในแท็บใบ) ทำ label จาง (`opacity:.45`) + `:title` tooltip แทนการซ่อน field (ต่างจาก pattern `v-if` ซ่อนทั้ง field ที่ production/dashboard ใช้เดิม เพราะต้องให้ผู้ใช้เตรียมค่าไว้ก่อนสลับแท็บได้); mapping tab→endpoint เดิมยังคงคอลัมน์/logic เดิมทั้งหมด ยกเว้นมุม "ชนิดทอง" ในแท็บ Stage ที่ตัดช่องเงิน/ปุ่มบันทึกค่าตั้งต้นทิ้ง (เหลือ endpoint `GoldLossMonthlyReport` ไว้ยิงเฉพาะคอลัมน์น้ำหนัก); route เดิม 6 เส้น (`/report-gold-loss-*`) เปลี่ยนจาก `component` เป็น `redirect` string พร้อม query (เช่น `?tab=stage&group=gold`) + `minorShow:false` (เมนูอ่านจาก field นี้อยู่แล้ว ไม่ต้องลบ route entry) — blueprint: `docs/claude-design/blueprints/gold-loss-dashboard.html` |
| 2026-09-11 | gold-loss-dashboard (แท็บภาพรวม, ปรับปรุง) + chart-colors/SourceStripGeneric | user feedback หลังดูหน้าจริง: (1) ถอด `roseGoldLight`/`roseGoldDark` ออกจาก `CHART_TOKENS` — rose-gold สงวนไว้ให้ `--nav-active-gradient` เท่านั้น ไม่ใช่สีกราฟ, เพิ่ม Core Principle #13 บังคับกราฟทุกตัวใช้ `CHART_PALETTE` (5 สี) เท่านั้น; (2) กราฟ "Loss รายเดือนแยกช่าง" (stacked column 7 สีต่อเดือน) → **ranking bar สีเดียว** (`CHART_TOKENS.primary`) เรียง loss รวมทั้งช่วงมาก→น้อย ทุกช่างในแผนก (ตัด 12 คนแรก ที่เหลือรวม "อื่นๆ") — เหตุผล: palette มี 5 สีไม่พอกับจำนวนช่าง และเพิ่มสีนอก palette ผิดหลัก sync กับ `variable.scss`; เปลี่ยนหัวข้อกล่องเป็น "อันดับ Loss ต่อช่าง"/"Loss Ranking by Worker" ทั้ง th/en (คีย์เดิม `chartSlipByWorkerTitle`); (3) default ช่วงวันที่ของ shared filter (`index-view.vue`) เปลี่ยนจาก `null` (ไม่กรอง) เป็น **6 เดือนล่าสุด** (เดือนปัจจุบัน+ย้อนหลัง 5 เดือน, ขอบเดือนตามเวลาไทย `Asia/Bangkok`) ทั้งตอนเข้าหน้าครั้งแรกและตอนกด "ล้างตัวกรอง" — ทำให้ zero-fill เดือนที่ไม่มีใบ (logic เดิมมีอยู่แล้วแต่ไม่เคยทำงานเพราะ filter ว่างตลอด) เห็นผลจริงโดยไม่ต้องแก้ logic กราฟ/ตารางเพิ่ม; (4) จัดหน้าให้ตรงต้นแบบ `/ticket-manage`: kpi-source-header เขียนเอง (Core Principle #2 ห้าม) → `SectionCardGeneric headerStyle="legend"` ครอบ kpi-grid ทั้ง 2 กลุ่ม (PLAN accent=main / SLIP accent=green, ไอคอน+accent แทนป้าย PLAN/SLIP เดิม), กราฟเทียบ 2 แผนก + กราฟอันดับใหม่จัดเป็น 2 คอลัมน์ (`.charts-row-b`, ≤1024px ยุบ 1), ป้าย PLAN/SLIP มุมขวาบนการ์ด KPI คงไว้ตามที่ user สั่งแต่ย่อเล็กลง (10px→9px ตัวหนา) และเปลี่ยนจากป้ายพื้นทึบ+ตัวอักษรขาวเป็น outline chip โปร่งแสง; (5) `SourceStripGeneric` เปลี่ยนจากแถบพื้นสีเต็มความกว้าง (highlight/green/progress bg) เป็นบรรทัดคำอธิบายเล็กใต้หัวข้อ (เส้นใต้บาง + badge outline สีตาม accent) เบาลงทุกแท็บที่ใช้ component นี้ร่วมกัน — เพิ่มหัวข้อ [Reference Layout — Dashboard](#reference-layout--dashboard-canonical--หน้า-ticket-manage) canonical จาก `ticket-dashboard.vue`/`ticket/manage/index-view.vue` |
| 2026-09-15 | sale/pipeline-dashboard (rebuild) | สร้างใหม่ทั้งหน้าเป็น dashboard ยอดขายจาก invoice (แทนของเดิมที่อิง quotation pipeline) ตาม pattern gold-loss-dashboard — `DashboardHeaderGeneric` + `SearchBarGeneric` (วันที่ออกบิล/จุดขาย/ลูกค้า) + `SectionCardGeneric headerStyle="dashboard"` ครอบ KPI (`.kpi-grid`/`.kpi-card`) + `SectionCardGeneric headerStyle="legend"` ครอบกลุ่มสินค้าขายดี (`ToggleGroupGeneric` แยกตาม + `.charts-row-b` กราฟอันดับ+ตาราง); สร้าง generic ใหม่ `ToggleGroupGeneric` สกัดจาก gold-loss-dashboard `.group-by-toggle` (ใช้ซ้ำครั้งที่ 3 หลัง overview-tab-view + stage-tab) — เป็น generic component แล้ว แต่ **gold-loss-dashboard ยังไม่ migrate มาใช้** (ไฟล์นั้นถูก session อื่นแก้ค้างอยู่) รอ migrate รอบถัดไป |
| 2026-09-14 | SectionCardGeneric (global) + SourceStripGeneric + ChartGeneric | กลุ่มการ์ด KPI บน dashboard ใช้ `SectionCardGeneric headerStyle="dashboard"` (หัวข้อ + เส้น ไม่มีกรอบครอบ) ส่วน `StatCardGeneric` ยังมีกรอบของตัวเอง — เหตุผล: legend box ครอบ StatCard ทำให้กรอบซ้อน 2 ชั้น; กราฟ/ตารางยังใช้ `headerStyle="legend"` เหมือนเดิม — reference file: `src/views/production/gold-loss-dashboard/components/overview-tab-view.vue`; `SourceStripGeneric` เพิ่ม source `'slip'` (ทั้ง 2 แผนกช่างแต่ง+ช่างฝังรวมกัน) ต่อจาก `'slip-tang'`/`'slip-setter'` เดิม; `ChartGeneric` แก้บั๊กกราฟหายเมื่อ series เปลี่ยนจำนวนแท่ง (bar count) แล้วดัน height เปลี่ยนตาม — เพิ่ม `:key="` `${type}-${height}` `"` บังคับ remount กัน vue3-apexcharts `refresh()` (destroy+async re-init) แข่งกับ watcher `options`/`series` ที่เรียก `updateOptions`/`updateSeries` บน instance ที่กำลังถูก destroy จนโยน `TypeError` แล้วกราฟหายไปเฉยๆ (เจอจริงที่กราฟ "อันดับ Loss ต่อช่าง" ตอนสลับแผนก); พร้อมแก้ `isEmpty` ให้อ่าน data point ที่เป็น object (`{x,y,goals}`) ผ่าน `.y` แทนที่จะ `Number(object)` ได้ `NaN` แล้วตีความผิดว่า series ว่างเปล่า |
| 2026-09-16 | dashboard/home (gold price tab) | แท็บ "ราคาทองวันนี้" กรอบซ้อน 2 ชั้น (user ทัก) — แก้ตามมาตรฐาน Decision Log 2026-09-14: กล่อง "ราคาประกาศวันนี้" ที่ครอบ `StatCardGeneric` 4 ใบ เปลี่ยน `headerStyle="legend"` → `"dashboard"` และ 4 การ์ดสถิติใต้กราฟ (สูงสุด/ต่ำสุด/เฉลี่ย/เปลี่ยนแปลงทั้งช่วง) ย้ายออกมานอกกล่องกราฟเป็น `.kpi-grid` ลอย; grid ของทั้ง 2 ชุดใช้ `.kpi-grid`/`.kpi-card` เหมือน `sale/pipeline-dashboard/components/summary-view.vue` (4 คอลัมน์ → 2 ที่ ≤1024px, `align-items: stretch`, `:deep(.stat-card){height:100%}`) แทน grid ที่เขียนเอง; กล่องกราฟ ApexCharts และกล่องตารางราคารายวัน ยังเป็น `headerStyle="legend"` ตามเดิม — ไฟล์: `src/views/dashboard/home/components/gold-price-panel.vue`, `gold-price-chart.vue` |
| 2026-09-21 | sale/invoice (list) + BaseDataTable (global) | (1) search bar ตัดส่วนที่ 3 advanced dialog ทิ้ง เปลี่ยนเป็น `SearchBarGeneric` grid ตายตัว 2 บรรทัด (override `.form-col-container`: 5 คอลัมน์ ที่ ≥1400px / 3 คอลัมน์ 1024–1399px / 1 คอลัมน์ <1024px) ฟิลด์รอง (dropdown ผู้สร้างเอกสาร + checkbox ค้างชำระ) ย้ายไป slot `#actions-left` — variant ใหม่นี้ใช้เมื่อ field เยอะแต่ไม่อยากซ่อนหลัง dialog ดู `ui-layout` skill §1.1; (2) `BaseDataTable` เพิ่ม prop `columnPrefsKey` — จำลำดับคอลัมน์ (ลาก `reorderableColumns`) + การปักหมุด (panel ⚙ `showColumnSettings`) ลง `localStorage` (`table-cols-${key}-dk`) พร้อมปุ่มรีเซ็ตใน panel — ว่าง (default) = พฤติกรรมเดิมทุกประการ ไม่กระทบตารางอื่น; ห้ามใช้ `stateStorage`/`stateKey` ของ PrimeVue เอง (ชนกับ `useDataTablePaging`) — helper แยกที่ `src/services/utils/column-prefs.js`, ตัวอย่างใช้จริง `sale/invoice/components/data-table-view.vue` — ดู `generic-components` skill; (3) ปุ่มดูรายละเอียดในหน้า list เปลี่ยนจากเปลี่ยนหน้าเดิมเป็นเปิด detail ใน **tab ใหม่** (`this.$router.resolve()` + `window.open(href, '_blank', 'noopener')`) ยังคงเรียก `setActiveRow(data)` ก่อนเสมอเพื่อให้ active-row highlight ทำงานเมื่อกลับมาที่ tab list |
| 2026-09-21 | setting/barcode-printer | ถอด `border-left`/`border-left-color` accent ออกจากกล่องสถานะ (`.status-box`) เหลือกรอบบาง 1px เท่ากันทุกด้าน — ย้ายการสื่อสถานะเขียว/เหลือง/แดงไปที่สีไอคอน + สี `.status-box-title` แทน; เพิ่ม Core Principle #14 ห้ามแถบสีหนาด้านซ้ายในกล่องสถานะ/callout ทุกที่; พร้อมแก้ `view.setting` mount ใน `src/language/th.js`/`en.js` ที่ก่อนหน้านี้ mount แค่ `companyInfo` ทำให้ `view.setting.barcodePrinter.*`/`view.setting.breakdown.*` ขึ้นเป็นชื่อ key — เปลี่ยนเป็น `{ ...settingLang }` ทั้งคู่ |
| 2026-09-22 | mobile stock-product (customer gallery, ใหม่) + public showcase | เพิ่มระบบรูปสำหรับลูกค้าแยกจากรูปภายในเดิม (`imagePath`/`ImagePreview type="STOCK-PRODUCT"` คงเดิมทุกที่) — 2 scope: **MOLD** (ใช้ร่วมทุกชิ้นของแบบ) / **SKU** (เฉพาะชิ้นนี้), resolved order = SKU ก่อน MOLD สูงสุด 4 รูป; `detail-info.vue` เพิ่มการ์ด "รูปสำหรับลูกค้า" (thumbnail + caption sku/mold count + ปุ่ม "จัดการรูป" หลัง `PermissionService.hasPermission(PERMISSIONS.STOCK_PRODUCT_GR_IMAGE_CREATE)` — ไม่ใช้ `authStore.hasPermission`); หน้าใหม่ `mobile-stock-product-photos` (`photos-view.vue` + `components/photo-grid.vue` + `components/photo-action-sheet.vue`) — grid 4 ช่องต่อ scope + tile เพิ่มรูป, sticky bottom bar (ถ่ายรูป/เลือกรูป + `RadioGroupGeneric` เลือก scope ปลายทาง), **sequential upload tile pattern**: คิวทีละไฟล์ (`status: queued→uploading→done/error`) แต่ละ tile มี progress bar ของตัวเอง + retry/remove เมื่อ error, จบ batch ค่อย re-fetch `Get` ครั้งเดียว; tap รูป → action sheet (ตั้งรูปหลัก/เลื่อนซ้าย/เลื่อนขวา/ลบ ผ่าน `Reorder`/`Delete`) ใช้ pattern Teleport full-screen sheet เดิมของแอป (เหมือน `filter-sheet.vue`/`payment-record-sheet.vue`) ไม่สร้างระบบ modal ใหม่; upload ผ่าน `skipLoading: true` (flag ที่มีอยู่แล้วใน `axios-helper.js`) กัน global loading overlay บังจอระหว่างมี progress bar ในตัว; เพิ่ม `compressGalleryImage()` ใน `compress-image.js` (decode ตาม EXIF orientation ก่อนเสมอ, ปฏิเสธไฟล์ด้านยาว <1000px ห้าม upscale, resize ด้านยาว min(ต้นฉบับ,1600) q0.85→0.7 ถ้า >600KB); public showcase (`product-showcase.vue`) ตัด fallback `imagePath` ออกจาก `images` computed เหลือ `product.images` (gallery) ล้วน |
| 2026-09-22 | Public showcase (/p/:token) redesign — "Ivory Boutique" | (1) เพิ่ม `--showcase-*` palette (10 สี + mask/typography/radius/sizing รวม ~38 token) ใน `variable.scss` **scoped เฉพาะหน้าลูกค้า** (`--showcase-bg` ivory #FBF8F5, `--showcase-accent` rose-gold เข้ม #A4604F ใช้กับ overline/icon พลอย, `--showcase-line`/`-line-soft`/`-box-border`/`-tile` เป็นเส้น/พื้นโทนอบอุ่นแยกจาก `--color-border`/`--color-highlight-bg` กลางระบบ) — เหตุผลที่แยก token: หน้านี้ไม่ใช้ chrome ผู้ดูแลระบบ (ไม่มี sidebar/topbar, ดู decision 2026-09-08) จึงมี mood/โทนสีเป็นของตัวเองไม่ผูกกับ `--base-*`/`--color-*` ที่ออกแบบไว้สำหรับพื้นหลังขาวของ back office; primary (`var(--base-font-color)`) ยังใช้ร่วมสำหรับราคา/ปุ่มแชร์/active ring เพื่อคง brand identity เดียวกันทั้งระบบ; (2) self-host font เฉพาะหน้านี้ผ่าน `@fontsource/taviraj` (500/600), `@fontsource/ibm-plex-sans-thai` (400/500/600), `@fontsource/ibm-plex-mono` (600) — import เป็น side-effect ใน `index-view.vue` script (ไม่ใช่ global main.js) เพื่อให้ไฟล์ font โหลดเฉพาะ route chunk นี้ ไม่กระทบ bundle ของหน้า back office; (3) เพิ่ม `applyLocale(lang)` ใน `plugins/i18n/config.js` — ตั้ง locale ในหน่วยความจำอย่างเดียว ไม่เขียน `storage`/`lang` key, แยกจาก `setLocale()` เดิม (ยังคงพฤติกรรมเดิมทุกประการ เรียก `applyLocale` แล้วค่อย persist) — ใช้แก้ปัญหา staff เปิดหน้าโชว์สินค้าบนอุปกรณ์หน้าร้านแล้วดันเปลี่ยนภาษาระบบหลังบ้านของทั้งเครื่อง: หน้า public เปิดเป็น EN เสมอ (`created()`) และคืน locale ระบบเดิมตอนออกจากหน้า (`beforeUnmount()` อ่านจาก `storage.getItem('lang','th')`) — ตัวสลับภาษาบน header หน้านี้ก็เรียก `applyLocale` เท่านั้น ไม่แตะ storage; (4) รวมโค้ดตัดรหัสสินค้าเหลือชุดเดียว (ตัด `codeNewLabel`/`productNumber` ออกจาก UI แต่ไม่ลบ i18n key เพราะยังไม่ยืนยันว่าไม่ใช้ที่อื่น), ย้าย availability pill ออกจาก `showcase-spec.vue` มาอยู่ใต้กรอบราคาใน `product-showcase.vue`; gallery รองรับ `images[]` สูงสุด 4 รูปพร้อม counter/thumbnails (API ปัจจุบันยังส่งรูปเดียว); เพิ่ม `src/config/public-product-type-config.js` map ชื่อประเภทสินค้าภาษาไทยจาก API (`productTypeName`) → i18n key (`view.public.showcase.productType.*`) เพราะ API ไม่มีฟิลด์อังกฤษ — ไม่รู้จัก + locale th แสดง raw text, ไม่รู้จัก + locale en ซ่อน overline ไปเลย (กันข้อความไทยหลุดในโหมด EN) |
| 2026-09-22 | stock/product/image (web, ใหม่) + mobile stock-product refactor | Round 2 ของ customer gallery — เพิ่มฝั่ง web ที่ `/goods-receipt-image`: (1) `IndexView.vue` ครอบด้วย `TabViewGeneric` 2 แท็บ sync กับ query string `?tab=customer\|internal` (pattern เดียวกับ `gold-loss-dashboard/index-view.vue` — อ่าน query ครั้งเดียวใน `created()` + `$router.replace` ตอน tab เปลี่ยน) แท็บ `internal` ครอบ `create-view.vue`/`image-view.vue` เดิมไว้เฉยๆ ไม่แตะเนื้อหา; (2) เพิ่มชุด component ใหม่ใต้ `components/customer-gallery/`: `customer-gallery-view.vue` (orchestrator ถือ state gallery/pendingUploads ทั้งหมด) → `scan-search-row.vue` (ช่องยิงบาร์โค้ด/เลขสต็อก/เลขเก่า + ปุ่มค้นหา, `<form>` ธรรมดาให้ Enter จากเครื่องสแกนใช้งานได้) → `gallery-editor.vue` (info line + 2 `SectionCardGeneric headerStyle="legend"` ต่อ scope ตาม Core Principle #2 — ซ่อนกลุ่ม MOLD เมื่อ `!canUseMoldScope`) → `gallery-photo-group.vue` (drop zone + tile grid ต่อ scope, ใหม่ทั้งหมด เทียบเท่า mobile `photo-grid.vue`+`photo-action-sheet.vue` รวมกันแต่ปรับสำหรับ desktop) → `missing-list.vue` (backlog "แบบที่ยังไม่มีรูปลูกค้า" = `SearchBarGeneric` + `MultiSelectGeneric` ประเภทสินค้า (ตัวเลือกจาก `useMasterApiStore().fetchProductType()` เดิม) + `DataTableWithPaging`/`useDataTablePaging` มาตรฐาน); (3) **web drag-reorder ใหม่ (native HTML5 DnD ไม่ใช้ library เพิ่ม)** — แยก 2 กรณีจาก `dataTransfer.types`: มี `'Files'` = ลากไฟล์จาก OS มาอัปโหลด (highlight dropzone ทั้งกล่อง), ไม่มี = ลาก tile เดิมมาจัดเรียง (ใช้ custom mime `application/x-gallery-tile-index` กัน false-positive กับไฟล์ OS, highlight เฉพาะ tile เป้าหมายด้วย `dragenter`); ทุก tile มีปุ่มจริงสำรอง (ตั้งรูปหลัก/เลื่อนซ้าย/เลื่อนขวา/ลบ) โผล่ตอน `:hover`/`:focus-within` สำหรับ keyboard user — ลบใช้ `confirmThenSubmit`; (4) สกัด logic ที่ใช้ร่วมกับ mobile ออกเป็น `src/services/helper/gallery/gallery-helpers.js` (ใหม่, มี spec คู่กัน): `buildGalleryPositionMap`/`classifyGalleryPosition` (แยก badge type ออกจาก label ข้อความ — caller ใส่ $t เอง), `calcFreeSlots`, `runUploadQueue` (sequential queue runner รับ items+uploadFn+onError, mutate item ตรงๆ), `moveIdToIndex`/`moveIdToFront`/`swapIds` (reorder helpers) — `photos-view.vue` และ `photo-grid.vue` (mobile) แก้ให้เรียก helper ชุดนี้แทนโค้ดเดิมที่ inline ไว้ ผลลัพธ์/UI มือถือไม่เปลี่ยน เป็น refactor ภายในล้วนๆ; (5) เพิ่ม `fetchMissingList` ใน `product-gallery-api.js` store ยิง `StockProductGallery/MissingList`; (6) i18n namespace ใหม่ `view.stock.productGallery` (mount ผ่าน `stock.productGallery` ใน `language/th.js`/`en.js` เหมือน `stock.product` เดิม) reuse key ข้อความที่เหมือนกับ mobile ตรงๆ จาก `view.mobile.stockProductPhotos.*` (codeHint, badge, warning, confirm delete, action label) แทนสร้างซ้ำ — **contract assumption**: `MissingList` response อาจแถม `totalMissingMoldCount`/`totalMissingPieceCount` (ไม่บังคับ) สำหรับข้อความ "เหลือ N แบบ / M ชิ้น" ถ้า backend ตั้งชื่อ field ต่างจากนี้ให้แก้ที่ `missing-list.vue` computed `summaryText` จุดเดียว |
| 2026-09-23 | receipt-stock/product/dashboard (rebuild) + SectionCardGeneric (global) | สร้างใหม่ทั้งหน้า `/stock-product-dashboard` แทนของเดิม (tab overview/today/weekly/monthly) ตาม pattern `sale/pipeline-dashboard` — `DashboardHeaderGeneric` + `SearchBarGeneric` (คลัง/ประเภทสินค้า/ทอง/ขนาดทอง/ช่วงยอดขายที่ใช้เทียบ) + 4 บล็อก: (1) ภาพรวมคลัง `headerStyle="dashboard"` ครอบ `StatCardGeneric` ×4, (2) สต็อกแยกตามกลุ่ม `headerStyle="legend" accent="main"` (`ToggleGroupGeneric` + กราฟแท่งนอน top12+"อื่นๆ" + ตารางทุกกลุ่ม, ลอก `product-group-view.vue`), (3) สต็อกค้างนาน `headerStyle="legend" accent="warning"` (เกณฑ์อายุ 1/2/3/5 ปี + กราฟ 6 ช่วงอายุ + ตาราง topGroups + ตารางรายชิ้น paginated พร้อม export Excel), (4) สมดุลสต็อก vs ยอดขาย `headerStyle="legend" accent="green"` (กราฟแท่งคู่ %สต็อก/%ยอดขาย + ตาราง badge สถานะ over/balanced/under/nosale + การ์ดรูปแบบที่ต้องดูแล เหลือน้อย/ผลิตเกิน ลอก `top-design-grid.vue`) — **ไม่มีมิติมูลค่า/ราคาในหน้านี้เลยตามที่ user ตัด scope**, ทุกตัวเลขเป็นชิ้น/แบบ/วัน/%; เพิ่ม **`SectionCardGeneric` accent variant ใหม่ `'warning'`** (legend + dashboard mode, สี `var(--base-warning)`) เพราะบล็อก (3) เป็นการแจ้งเตือนสต็อกค้าง ไม่ใช่ primary/success — backward compatible, `'main'`/`'green'` เดิมไม่เปลี่ยน; store ใหม่ `stock-report-api.js` (6 actions: Summary/ProductGroup/Aging/AgingItems/ProductionBalance/DesignAlerts, ทุกตัว request-sequence guard ตาม `sale-report-api.js`) — `filterOptions` (คลัง/ประเภทสินค้า/ทอง/ขนาดทอง) มาจาก `/Summary` เท่านั้น เก็บใน store ให้ `search-view.vue` อ่านไปทำ dropdown; รูปการ์ด "แบบที่ต้องดูแล" ประกอบ blob path เองฝั่ง FE จาก `imagePath`/`imageName` แยกกัน (พอร์ตจาก backend helper `StockImagePath.Build` เพราะ contract endpoint นี้ไม่ได้ pre-assemble path มาให้เหมือน `imageBlobPath` ของ pipeline-dashboard) — ลบไฟล์เดิมทั้งชุด: `dashboard-view.vue`, `components/category-chart.vue`/`last-activities-table.vue`/`stock-summary-cards.vue`, `stock-product-dashboard-store.js` (⚠️ endpoint เก่า `/StockProduct/Dashboard` ยังอยู่ — `dashboard/home/home-dashboard-store.js` ยังเรียกอยู่ ห้ามลบฝั่ง backend) |
