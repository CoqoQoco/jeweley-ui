...
| **mold** | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | 23 | |
| **production-cost** | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | 24 | |
| **production/plan-update** | ✅ | ⬜ | ✅ | ✅ | ✅ | 20* | Pass A+B ครบ; *20 primevue exceptions (eslint-disable ครบ); update/ 7 ไฟล์ + form-status/ 3 ไฟล์ migrate ครบ: Calendar/Dropdown/AutoComplete→Generic, input→InputTextGeneric, textarea→TextareaGeneric, swAlert→named imports, confirmThenSubmit, try-catch/isLoading/console.log removed, localStorage→storage.getJSON; i18n planUpdate.* 22 keys เพิ่ม th+en คู่กัน |
